...
public class PlaywrightPageTools {
    ...
    @McpTool(
            description = "Open a URL in the browser. Use this as the first step for live site work, then call pageSnapshot to inspect the page and choose locators.",
    ...
    @McpTool(
            description = "Inspect the current page for orientation. Use a full body snapshot to discover what exists after navigating, logging in, or opening a menu, and to choose locators. Returns compact accessibility YAML for page STRUCTURE (headings, landmarks, links, text, roles) plus locator targets. Grids and data-like tables are collapsed to a one-line summary by default so broad snapshots stay useful, and the collapse field explains what was collapsed. Once you know the target locator, prefer locatorText and pageWaitForLocator over snapshotting the full body again. Pass a root locator such as nav, aside, main, a dialog, iframe body, table, or form when you only need one region. Extra detail flags add response size and time: includeControls=true returns interactive controls and states; includeTooltips=true also hovers icon-only controls for tooltips; includeGrids=true reads rendered rows and columns inside tables and ARIA grids/treegrids. Set collapseSnapshot=false only when you need the raw Playwright aria snapshot for a narrow locator or collapse debugging. The result reports matchedCount: the root locator should match one region. matchedCount=0 means it did not resolve and an empty snapshot is returned immediately without waiting, so fix the locator instead of retrying or raising the timeout; matchedCount>1 means the snapshot, controls and grids describe only the first match.",
            generateOutputSchema = true,
    ...
    public PageSnapshotResult pageSnapshot(
            ...
            @McpToolParam(description = "When true (and includeControls is true), hover icon-only/nameless controls to capture hover tooltip text. Slower; use only when controls show an icon code instead of a text label.", required = false) Boolean includeTooltips,
            ...
            @McpToolParam(description = "Maximum number of controls to return when includeControls is true. Default 80; larger values are capped.", required = false) Integer maxControls,
            @McpToolParam(description = "Maximum rendered rows per grid to return when includeGrids is true. Default 50; larger values are capped.", required = false) Integer maxRows,
